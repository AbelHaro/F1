...
export function Drivers() {
        ...
        if (nationalities.length === 0) {
            fetch(URL_NATIONALITIES)
                ...
                .then(data => {
                    ...
                    modifiedNationalities.unshift({ id: 'all', value: 'All Nationalities', name: 'All All Nationalities' }); // Agregar un valor al inicio
    ...
    useEffect(() => {
        if (drivers.length === 0) {
            fetch(`${URL_ALL_DRIVERS}`)
                .then(response => response.json())
                .then(drivers => {
                    const modifiedDrivers = drivers.map(driver => ({ id: driver.id, value: driver.name, name: driver.name}))
    ...
    function setSelectedNationalityDefault() {setSelectedNationality('all')}
    ...
    function handleChangeSelectedDriver(item) {
        ...
        const selectedDriverIndex = drivers.findIndex(driver => driver.id === item.id)
        setSelectedName(drivers[selectedDriverIndex].id)
    }

    const nationalitiesOptions = nationalities.map(nationality => (
        <option className='selector-label' key={nationality.id} value={nationality.id}>
    ...
    return (
        <main className='drivers-container'>
            <div className='input-main-container'>
                ...
                <div className='input-container'>
                    <h3 className='input-title'>Choose nationality</h3>
                    <select className='input' onChange={handleChangeSelectedNationality} value={selectedNationality}>
